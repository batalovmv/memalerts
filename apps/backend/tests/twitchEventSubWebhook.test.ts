...
function signEventSub(secret: string, messageId: string, timestamp: string, body: any): string {
...
describe('Twitch EventSub webhook /webhooks/twitch/eventsub', () => {
  ...
  it('rejects too-old timestamp even with valid signature', async () => {
    ...
    const sig = signEventSub(secret, messageId, oldTs, body);
  ...
  it('accepts valid signature + timestamp for unknown events', async () => {
    const secret = process.env.TWITCH_EVENTSUB_SECRET!;
    const body = { subscription: { type: 'something.unknown' }, event: { ok: true } };
    const messageId = 'm3';
    const ts = String(Date.now());
    const sig = signEventSub(secret, messageId, ts, body);
