# MemAlerts Backend

Backend API для системы активации мемов через Twitch Channel Points. Express API с Socket.IO, PostgreSQL и автоматическим деплоем через GitHub Actions.

## 🚀 Текущий функционал

### ✅ Реализовано

- **Аутентификация через Twitch OAuth**
  - Авторизация пользователей через Twitch
  - Автоматическое создание каналов для стримеров
  - JWT токены в httpOnly cookies
  - Роли: viewer, streamer, admin

- **Управление мемами**
  - Загрузка мемов пользователями (заявки на одобрение)
  - Модерация заявок стримерами (одобрение/отклонение)
  - Активация мемов зрителями за виртуальные монеты
  - Поддержка типов: image, gif, video, audio

- **Система баланса**
  - Виртуальные монеты для каждого пользователя
  - Получение монет через Twitch Channel Points (EventSub webhooks)
  - Трата монет на активацию мемов
  - Настройка конвертации Channel Points → монеты

- **Профили стримеров**
  - Уникальные профили по slug канала
  - Публичный API для получения информации о канале
  - Автоматическое создание канала при первом входе стримера

- **Real-time коммуникация**
  - Socket.IO для overlay в OBS
  - Мгновенная отправка активированных мемов на overlay
  - Поддержка множественных каналов

- **Безопасность**
  - Rate limiting на активацию мемов (1 раз в 3 секунды)
  - Валидация всех данных через Zod
  - HMAC проверка EventSub webhooks
  - Ограничения на размер и тип файлов
  - CORS настроен для разрешенных доменов

- **Автоматический деплой**
  - CI/CD через GitHub Actions
  - Автоматическое создание .env из GitHub Secrets
  - Автоматическое применение миграций БД
  - Деплой на VPS с PM2

## 📋 API Endpoints

### Публичные

- `GET /health` - Проверка работоспособности API
- `GET /channels/:slug` - Получить информацию о канале по slug

### Аутентификация

- `GET /auth/twitch` - Инициация Twitch OAuth
- `GET /auth/twitch/callback` - Callback от Twitch OAuth
- `POST /auth/logout` - Выход из системы

### Пользователь (требует авторизации)

- `GET /me` - Информация о текущем пользователе
- `GET /wallet` - Баланс кошелька
- `GET /memes` - Список одобренных мемов (`?channelSlug=xxx` или `?channelId=xxx`)
- `POST /memes/:id/activate` - Активировать мем

### Заявки на мемы (требует авторизации)

- `POST /submissions` - Создать заявку на мем (с загрузкой файла)
- `GET /submissions/mine` - Получить мои заявки

### Админ панель (требует роль streamer/admin)

- `GET /admin/submissions` - Получить заявки (с фильтром по статусу)
- `POST /admin/submissions/:id/approve` - Одобрить заявку
- `POST /admin/submissions/:id/reject` - Отклонить заявку
- `GET /admin/memes` - Получить все мемы канала
- `PATCH /admin/memes/:id` - Обновить мем (цена, длительность)
- `PATCH /admin/channel/settings` - Обновить настройки канала (rewardIdForCoins, coinPerPointRatio)

### Webhooks

- `POST /webhooks/twitch/eventsub` - Webhook от Twitch EventSub для обработки Channel Points redemptions

## 🏗️ Структура проекта

```
.
├── prisma/
│   ├── schema.prisma    # Prisma схема базы данных
│   └── seed.ts          # Скрипт для заполнения тестовыми данными
├── src/
│   ├── controllers/     # Контроллеры для обработки запросов
│   │   ├── authController.ts
│   │   ├── viewerController.ts
│   │   ├── submissionController.ts
│   │   ├── adminController.ts
│   │   └── webhookController.ts
│   ├── middleware/      # Express middleware
│   │   ├── auth.ts
│   │   ├── errorHandler.ts
│   │   ├── rateLimit.ts
│   │   ├── upload.ts
│   │   └── requireChannel.ts
│   ├── routes/          # Маршруты API
│   │   ├── index.ts
│   │   ├── auth.ts
│   │   ├── viewer.ts
│   │   ├── submissions.ts
│   │   ├── admin.ts
│   │   └── webhooks.ts
│   ├── socket/          # Socket.IO настройка
│   │   └── index.ts
│   ├── lib/             # Утилиты
│   │   └── prisma.ts
│   ├── shared/          # Общие типы и Zod схемы
│   │   ├── types.ts
│   │   ├── schemas.ts
│   │   └── index.ts
│   └── index.ts         # Точка входа приложения
├── .github/
│   ├── workflows/
│   │   └── ci-cd.yml    # GitHub Actions CI/CD
│   └── scripts/         # Скрипты для настройки сервера
├── uploads/             # Загруженные файлы (мемы)
└── dist/                # Собранный JavaScript код
```

## 🚀 Быстрый старт

### Требования

- Node.js >= 18.0.0
- pnpm >= 8.0.0
- PostgreSQL >= 15

### Локальная разработка

```bash
# Установить зависимости
pnpm install

# Создать .env файл (см. раздел "Переменные окружения")
cp .env.example .env

# Настроить базу данных
pnpm db:push

# Запустить в dev режиме
pnpm dev
```

### Production деплой

Проект настроен на автоматический деплой через GitHub Actions. При push в ветку `main`:

1. Запускаются тесты
2. Собирается проект
3. Автоматически создается `.env` на сервере из GitHub Secrets
4. Код деплоится на VPS
5. Применяются миграции БД
6. Приложение перезапускается через PM2

## 📝 Переменные окружения

Все переменные окружения настраиваются через GitHub Secrets для автоматического деплоя:

### Обязательные GitHub Secrets

- `VPS_HOST` - IP адрес сервера
- `VPS_USER` - Пользователь для SSH (рекомендуется: `deploy`)
- `VPS_SSH_KEY` - Приватный SSH ключ
- `VPS_PORT` - Порт SSH (обычно `22`)
- `DATABASE_URL` - Строка подключения к PostgreSQL
- `JWT_SECRET` - Секретный ключ для JWT токенов
- `TWITCH_CLIENT_ID` - Client ID из Twitch приложения
- `TWITCH_CLIENT_SECRET` - Client Secret из Twitch приложения
- `TWITCH_EVENTSUB_SECRET` - Секрет для проверки EventSub webhooks
- `WEB_URL` - URL фронтенда
- `OVERLAY_URL` - URL overlay

### Опциональные GitHub Secrets

- `DOMAIN` - Доменное имя (используется для автоматического построения CALLBACK_URL)
- `TWITCH_CALLBACK_URL` - Явный URL для callback (если не указан, строится из DOMAIN)

### Cloudflare Origin Certificate (рекомендуется для HTTPS)

Если вы используете Cloudflare с прокси (оранжевое облако), добавьте:

- `CLOUDFLARE_ORIGIN_CERT` - Cloudflare Origin Certificate (полный сертификат с BEGIN/END)
- `CLOUDFLARE_ORIGIN_KEY` - Cloudflare Origin Private Key (полный ключ с BEGIN/END)

**Важно:** 
- Если вы добавите эти секреты, скрипт автоматически настроит HTTPS с Cloudflare сертификатом
- Если не добавите - будет использован Let's Encrypt (требует DNS-only режим в Cloudflare, серое облако)
- Cloudflare Origin Certificate работает с прокси (оранжевое облако) и не требует доступа к серверу из интернета

### Локальная разработка (.env)

```env
# Database
DATABASE_URL="postgresql://user:password@localhost:5432/memalerts?schema=public"

# Server
PORT=3001
NODE_ENV=development

# JWT
JWT_SECRET=your-super-secret-jwt-key
JWT_EXPIRES_IN=7d

# Twitch OAuth
TWITCH_CLIENT_ID=your-twitch-client-id
TWITCH_CLIENT_SECRET=your-twitch-client-secret
TWITCH_CALLBACK_URL=http://localhost:3001/auth/twitch/callback

# Twitch EventSub
TWITCH_EVENTSUB_SECRET=your-eventsub-secret

# CORS
WEB_URL=http://localhost:5173
OVERLAY_URL=http://localhost:5174

# File Upload
MAX_FILE_SIZE=10485760
UPLOAD_DIR=./uploads
```

## 📦 Команды

```bash
# Разработка
pnpm dev              # Запуск в dev режиме с hot reload

# Сборка
pnpm build            # Сборка TypeScript в JavaScript

# База данных
pnpm db:push          # Применить схему Prisma без миграций
pnpm db:migrate       # Создать и применить миграцию
pnpm db:seed          # Заполнить базу тестовыми данными
pnpm db:studio        # Открыть Prisma Studio (GUI для БД)

# Production
pnpm start            # Запуск собранного приложения
```

## 🔧 Настройка Twitch

1. Создайте приложение на https://dev.twitch.tv/console/apps
2. Получите Client ID и Client Secret
3. Добавьте Redirect URL:
   - Для локальной разработки: `http://localhost:3001/auth/twitch/callback`
   - Для production: `https://twitchmemes.ru/auth/twitch/callback`
4. Настройте EventSub подписку для `channel.channel_points_custom_reward_redemption.add`
5. Добавьте полученные данные в GitHub Secrets

## 🔮 Планы на будущее

### Ближайшие планы

- [ ] **Система поиска мемов**
  - Полнотекстовый поиск по названиям мемов
  - Фильтрация по типу (image, gif, video, audio)
  - Фильтрация по цене
  - Сортировка (по цене, дате создания, популярности)

- [ ] **Система тегов**
  - Добавление тегов к мемам
  - Фильтрация мемов по тегам
  - Популярные теги
  - Автоматические теги на основе анализа контента

- [ ] **Пополнение баланса**
  - Интеграция платежных систем (Stripe, PayPal)
  - Покупка монет за реальные деньги
  - Промокоды и бонусы
  - Реферальная система

- [ ] **Статистика и аналитика**
  - Статистика активаций мемов
  - Популярные мемы
  - Активность пользователей
  - Графики и отчеты для стримеров

- [ ] **Расширенная модерация**
  - Множественные модераторы на канал
  - Очередь модерации
  - Автоматическая модерация (AI)
  - Черные списки пользователей

### Долгосрочные планы

- [ ] **Множественные каналы для пользователя**
  - Пользователь может быть стримером нескольких каналов
  - Переключение между каналами
  - Общие мемы между каналами

- [ ] **Социальные функции**
  - Комментарии к мемам
  - Рейтинги мемов
  - Избранные мемы
  - История активаций

- [ ] **Мобильное приложение**
  - React Native приложение
  - Уведомления о новых мемах
  - Быстрая активация мемов

- [ ] **API для разработчиков**
  - Публичный API для интеграций
  - Webhooks для внешних сервисов
  - SDK для популярных языков

- [ ] **Монетизация**
  - Премиум подписки
  - Реклама в мемах
  - Партнерская программа

## 🐛 Troubleshooting

### Ошибка подключения к базе данных

- Убедитесь, что PostgreSQL запущен
- Проверьте правильность `DATABASE_URL` в `.env` или GitHub Secrets
- Убедитесь, что база данных `memalerts` создана

### EventSub не работает

- Убедитесь, что webhook URL публично доступен
- Проверьте, что `TWITCH_EVENTSUB_SECRET` совпадает с секретом в подписке
- Проверьте логи приложения: `pm2 logs memalerts-api`

### Файлы не загружаются

- Убедитесь, что директория `uploads/` существует
- Проверьте права доступа к директории
- Проверьте `MAX_FILE_SIZE` в `.env`

### Деплой не работает

- Проверьте, что все GitHub Secrets добавлены
- Проверьте SSH ключ и доступ к серверу
- Проверьте логи GitHub Actions
- Убедитесь, что на сервере установлены Node.js, pnpm, PostgreSQL, PM2

## 📄 Лицензия

MIT
