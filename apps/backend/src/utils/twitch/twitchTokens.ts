...
export async function refreshTwitchAccessTokenByExternalAccountId(externalAccountId: string): Promise<string | null> {
  const id = String(externalAccountId || '').trim();
  if (!id) return null;

  const row = await prisma.externalAccount.findUnique({
    where: { id },
    select: { id: true, provider: true, refreshToken: true, scopes: true },
  });
  if (!row || row.provider !== 'twitch') return null;
...
export async function refreshTwitchBotAccessToken(): Promise<{ accessToken: string; login: string } | null> {
  try {
    const cred = await prisma.globalTwitchBotCredential.findFirst({
      where: { enabled: true },
      orderBy: { updatedAt: 'desc' },
    ...
    const accessToken = await refreshTwitchAccessTokenByExternalAccountId(externalAccountId);
    if (!accessToken) return null;
    return { accessToken, login };
  } catch (error) {
    const prismaError = error as { code?: string; message?: string };
