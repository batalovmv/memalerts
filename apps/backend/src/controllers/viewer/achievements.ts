...
import { buildChannelStreamerAchievementsSnapshot, buildUserAchievementsSnapshot } from '../../services/achievements/achievementService.js';
...
export const getChannelStreamerAchievements = async (req: AuthRequest, res: Response) => {
  const slug = normalizeSlug(req.params.slug);
  if (!slug) {
    return res.status(400).json({ errorCode: ERROR_CODES.BAD_REQUEST, error: 'Channel slug required' });
  }
  ...
  const achievements = await buildChannelStreamerAchievementsSnapshot({ channelId: channel.id });
