...
**Что тестировать**:
- [ ] Успешный импорт по валидному URL
- [ ] Отклонение невалидного URL
- [ ] Отклонение слишком большого файла
- [ ] Отклонение невалидного content-type
- [ ] Timeout при долгом скачивании
- [ ] Дедупликация (тот же файл = reuse)
- [ ] Owner import → сразу в мемы
...
**Что тестировать**:
- [ ] `GET /wallet` — список всех кошельков пользователя
- [ ] `GET /channels/:slug/wallet` — кошелёк для конкретного канала
- [ ] Upsert при первом обращении (создание если нет)
- [ ] 401 без auth
...
**Что тестировать**:
- [ ] Топ мемов по активациям
- [ ] Кэширование (minute buckets)
