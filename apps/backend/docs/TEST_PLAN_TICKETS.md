...
- [x] Тест изолирован (не зависит от порядка выполнения)
- [x] Используются фабрики для создания тестовых данных
- [x] Мокируются внешние сервисы (Twitch, YouTube и т.д.)
- [x] Проверяется как happy path, так и error cases
- [x] Проверяются security invariants (см. `docs/TESTING.md`)
- [x] Тест проходит локально перед коммитом
