# 🔍 MemAlerts Backend — План проверки работоспособности

Чеклист для полной проверки состояния проекта.

---

## 1️⃣ Сборка и компиляция

### 1.1 TypeScript проверка
```bash
pnpm typecheck
```
**Ожидаемый результат:** Без ошибок (exit code 0)
**Фактический результат:** OK (npm warns: Unknown env config entries)

### 1.2 Сборка проекта
```bash
pnpm build
```
**Ожидаемый результат:** 
- Без ошибок
- Папка `dist/` обновлена
- Все файлы скомпилированы
**Фактический результат:** OK

### 1.3 Prisma генерация
```bash
npx prisma generate
```
**Ожидаемый результат:** `@prisma/client` сгенерирован успешно
**Фактический результат:** OK

---

## 2️⃣ Линтинг и форматирование

### 2.1 ESLint
```bash
pnpm lint
```
**Ожидаемый результат:**
- 0 ошибок (errors)
- Warnings ≤ 500 (текущий лимит для CI)
**Фактический результат:** OK

### 2.2 Prettier
```bash
pnpm format:check
```
**Ожидаемый результат:** Все файлы отформатированы
**Фактический результат:** FAIL (31 files need formatting)

---

## 3️⃣ Запуск сервера

### 3.1 Dev-режим (с hot-reload)
```bash
pnpm dev
```
**Ожидаемый результат:**
- Сервер запускается без ошибок
- Лог: `Server started on port XXXX`
- Нет `ERR_MODULE_NOT_FOUND`
- Нет `Route.XXX() requires a callback function`
**Фактический результат:** OK (dev server started; health endpoints return ok)

### 3.2 Production-режим
```bash
pnpm build && pnpm start
```
**Ожидаемый результат:** Аналогично dev-режиму
**Фактический результат:** FAIL (NODE_ENV=production requires DOMAIN; server refused to start)

---

## 4️⃣ Health-эндпоинты

После запуска сервера проверить:

### 4.1 Базовый health
```bash
curl http://localhost:3001/health
```
**Ожидаемый результат:** `{ "status": "ok" }`
**Фактический результат:** OK (dev)

### 4.2 Readiness (если есть)
```bash
curl http://localhost:3001/healthz
curl http://localhost:3001/readyz
```
**Фактический результат:** OK (dev)

---

## 5️⃣ База данных

### 5.1 Статус миграций
```bash
pnpm db:migrate:status
```
**Ожидаемый результат:** Все миграции применены
**Фактический результат:** FAIL (local DB has divergent migration history; 76 pending)

### 5.2 Prisma Studio (опционально)
```bash
pnpm db:studio
```
**Ожидаемый результат:** Открывается UI, таблицы доступны
**Фактический результат:** SKIP (UI not checked)

---

## 6️⃣ Тесты

### 6.1 Unit/Integration тесты
```bash
pnpm test
```
**Ожидаемый результат:** Все тесты проходят
**Фактический результат:** FAIL (3 tests: ownerMemeAssetModeration, ownerChannelResolve, viewerStats)

### 6.2 Coverage (опционально)
```bash
pnpm test:ci
```
**Ожидаемый результат:** Coverage не ниже baseline
**Фактический результат:** SKIP (base test run failed)

---

## 7️⃣ Проверка критических модулей

### 7.1 Socket.IO
- Подключиться к `ws://localhost:3001`
- Проверить события `join:channel`, `join:user`, `join:overlay`
**Фактический результат:** SKIP (manual check not executed; join behavior covered by tests)

### 7.2 OAuth провайдеры
- Проверить наличие env-переменных для каждого провайдера
- Twitch, YouTube, VKVideo, Trovo, Kick, Discord
**Фактический результат:** WARN (VKVIDEO_CLIENT_ID / VKVIDEO_CLIENT_SECRET missing)

### 7.3 Загрузка файлов
- POST `/submissions` с видео-файлом
- Проверить создание в `uploads/`
**Фактический результат:** SKIP (manual upload not executed)

---

## 8️⃣ Проверка на VPS (production/beta)

### 8.1 SSH подключение
```bash
ssh deploy@155.212.172.136
```
**Фактический результат:** OK

### 8.2 Статус PM2
```bash
pm2 status
```
**Ожидаемый результат:**
- `memalerts-api` - online (prod, порт 3001)
- `memalerts-api-beta` - online (beta, порт 3002)
**Фактический результат:** OK (prod + beta online)

### 8.3 Логи
```bash
pm2 logs memalerts-api --lines 50
pm2 logs memalerts-api-beta --lines 50
```
**Проверить:** Нет повторяющихся ошибок
**Фактический результат:** WARN (beta error log shows missing column MemeSubmission.aiProcessingStartedAt; prod logs empty)

### 8.4 Health на VPS
```bash
curl http://localhost:3001/health   # prod
curl http://localhost:3002/health   # beta
```
**Фактический результат:** OK

---

## 📋 Сводная таблица

| # | Проверка | Команда | Статус |
|---|----------|---------|--------|
| 1 | TypeScript | `pnpm typecheck` | OK |
| 2 | Сборка | `pnpm build` | OK |
| 3 | Prisma | `npx prisma generate` | OK |
| 4 | ESLint | `pnpm lint` | OK |
| 5 | Prettier | `pnpm format:check` | FAIL |
| 6 | Dev-сервер | `pnpm dev` | OK |
| 7 | Health | `curl /health` | OK (dev) |
| 8 | Миграции | `pnpm db:migrate:status` | FAIL |
| 9 | Тесты | `pnpm test` | FAIL |
| 10 | PM2 (VPS) | `pm2 status` | OK |

---

## ⚠️ Известные проблемы (на момент создания)

### ESLint Warnings
- **Количество:** ~960
- **Тип:** `@typescript-eslint/no-explicit-any`
- **Влияние:** Не блокирует работу, но может блокировать CI при `--max-warnings 500`

### Исторические логи
- Файл `tmp-runtime/api.err.log` содержит старые ошибки
- **Рекомендация:** Удалить перед проверкой, чтобы видеть только актуальные

```bash
rm tmp-runtime/api.err.log
```

### Проверка 2026-01-17
- Prettier: 31 files need formatting (`pnpm format:check` failed).
- Migrations: local DB migration history diverges; 76 pending migrations.
- Tests: 3 failures (ownerMemeAssetModeration, ownerChannelResolve, viewerStats).
- VPS beta logs: `MemeSubmission.aiProcessingStartedAt` column missing (schema mismatch).

---

## 🚀 Quick Start

Минимальная проверка за 2 минуты:

```bash
# 1. Сборка
pnpm build

# 2. Запуск
pnpm start &

# 3. Проверка health
sleep 3 && curl http://localhost:3001/health

# 4. Остановка
pkill -f "node dist/index.js"
```

---

*Последнее обновление: 2026-01-17*

