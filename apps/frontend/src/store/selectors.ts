...
export const selectMemeById = createSelector(
  [selectMemes, (_: RootState, id: string) => id],
  (memes, id) => memes.find((m) => m.id === id),
);
...
export const selectMemesCount = createSelector([selectMemes], (memes) => memes.length);
