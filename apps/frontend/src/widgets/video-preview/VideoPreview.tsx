...
export default function VideoPreview({ src, variants, title, className = '' }: VideoPreviewProps) {
  const [error, setError] = useState<string | null>(null);

  const normalizedSrc = (src || '').trim();
  const fullVariants = Array.isArray(variants) ? variants : [];
  const hasVariants = fullVariants.length > 0;
  const hasSrc = normalizedSrc.length > 0 || hasVariants;

  // Construct full URL - handle both relative and absolute URLs
  const getVideoUrl = () => {
    if (!hasSrc) return '';
    if (hasVariants) return resolveMediaUrl(fullVariants[0]?.fileUrl || '');
    ...
    <div className={`relative ${className}`}>
      <div className="bg-gray-900 rounded-lg overflow-hidden">
        {!hasSrc ? (
          ...
          <video
            src={!hasVariants ? videoUrl : undefined}
            controls
